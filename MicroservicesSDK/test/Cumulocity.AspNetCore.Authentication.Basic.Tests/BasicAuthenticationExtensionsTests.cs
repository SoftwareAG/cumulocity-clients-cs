...
namespace Cumulocity.AspNetCore.Authentication.Basic.Tests
{
    public class BasicAuthenticationExtensionsTests
    {
        ...
        public Task AddBasicAuthentication_VerifyDefaultValuesOnFirstFuncOverload()
        {
            ...
            services.AddAuthentication().AddBasicAuthentication(a => Task.FromResult(new BasicAuthenticationResult(){ IsAuthenticated = false }));
        ...
        {
            var services = new ServiceCollection();
            services.AddAuthentication().AddBasicAuthentication(o => { }, a => Task.FromResult(new BasicAuthenticationResult() { IsAuthenticated = false }));
            return AssertConfiguration(services, true);
        }
        ...
        public Task AddBasicAuthentication_VerifyAuthenticationSchemeOnFuncOverloadIsOverriden()
        {
            var services = new ServiceCollection();
            const string testScheme = "TestScheme";
            services.AddAuthentication().AddBasicAuthentication(testScheme, o => { }, a => Task.FromResult(new BasicAuthenticationResult() { IsAuthenticated = false }));
        ...
        public Task AddBasicAuthentication_VerifyAuthenticationSchemeAndDisplayNameOnFuncOverloadAreOverriden()
        {
            ...
            const string testScheme = "TestScheme";
            const string testDisplayName = "TestDisplayName";
            services.AddAuthentication().AddBasicAuthentication(testScheme, testDisplayName, o => { }, a => Task.FromResult(new BasicAuthenticationResult() { IsAuthenticated = false }));
